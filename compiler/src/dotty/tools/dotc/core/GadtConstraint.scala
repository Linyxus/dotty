...
final class ProperGadtConstraint private(
  ...
  override def fullUpperBound(param: TypeParamRef)(using Context): Type = trace("fullUpperBound inner", gadts, show = true) {
    trace(i"constraint.minUpper of $param", gadts, show = true) { constraint.minUpper(param) }.foldLeft(trace(i"nonParamBounds.hi of $param", gadts, show = true) { nonParamBounds(param).hi }) { (t, u) =>
