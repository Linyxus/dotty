...
import config.Printers.{constr, subtyping, gadts, matchTypes, noPrinter, scopedGadt}
...
class TypeComparer(@constructorOnly initctx: Context) extends ConstraintHandling, PatternTypeConstrainer {
  ...
  protected def gadtBounds(ntp: NamedType)(using Context) = trace(i"gadtBounds($ntp)", gadts, show = true) {
  ...
  def glb(tp1: Type, tp2: Type): Type = /*>|>*/ trace(s"glb(${tp1}, ${tp2})", scopedGadt, show = true) /*<|<*/ {
    if (tp1 eq tp2) tp1
