...
object DesugarEnums {
  ...
  def ordinalMeth(body: Tree)(using Context): DefDef =
    DefDef(nme.ordinal, Nil, Nil, TypeTree(defn.IntType), body).withMods(Modifiers(Synthetic))

  def enumLabelMeth(body: Tree)(using Context): DefDef =
    DefDef(nme.enumLabel, Nil, Nil, TypeTree(defn.StringType), body).withMods(Modifiers(Synthetic))
