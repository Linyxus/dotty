...
object DesugarEnums {
  ...
  final case class EnumConstraints(
      minKind: CaseKind.Value, maxKind: CaseKind.Value, enumValues: List[(Int, RefTree)], valueCount: Int):
    require(minKind <= maxKind && !(cached && enumValues.isEmpty))
   ...
   */
  val EnumCaseCount: Property.Key[(Int, CaseKind.Value, CaseKind.Value, List[(Int, TermName)], Int)] = Property.Key()
  ...
  private def enumLookupMethods(constraints: EnumConstraints)(using Context): List[Tree] =
    def scaffolding: List[Tree] =
      if constraints.isEnumeration then enumScaffolding(constraints.enumValues.map(_._2)) else Nil
    ...
    def fromOrdinal: Tree =
      ...
      if !constraints.cached then
        ...
        else if constraints.valueCount <= 10000 then
          fromOrdinalMeth(ordinal =>
            Match(ordinal,
              constraints.enumValues.map((i, enumValue) => CaseDef(Literal(Constant(i)), EmptyTree, enumValue))
              :+ default(ordinal)))
        else
          ???
  ...
  def nextOrdinal(name: Name, kind: CaseKind.Value, definesLookups: Boolean)(using Context): (Int, List[Tree]) = {
    val (ordinal, seenMinKind, seenMaxKind, seenValues, seenCount) =
      ctx.tree.removeAttachment(EnumCaseCount).getOrElse((0, CaseKind.Class, CaseKind.Simple, Nil, 0))
    ...
    if definesLookups then
      ...
      val cachedValues = values.reverse.map((i, name) => (i, Select(thisRef, name)))
      (ordinal, enumLookupMethods(EnumConstraints(minKind, maxKind, cachedValues, count)))
    else
      ctx.tree.pushAttachment(EnumCaseCount, (ordinal + 1, minKind, maxKind, values, count))
      (ordinal, Nil)
  }
